 ...
 * Serviço de Integração Geográfica (Multi-Provider com Fallback Resiliente)
 * Suporte a Estados, Municípios, Bairros, Ruas e CEP (ViaCEP, BrasilAPI, OpenCEP, IBGE)
 * ============================================================================
 */
...
class LocationService {
  ...
  async fetchComTimeout(url, timeoutMs = 3500) {
  ...
  /**
   * Busca endereço completo por CEP com Multi-Provider Fallback (ViaCEP -> BrasilAPI -> OpenCEP -> AwesomeAPI)
  ...
  async buscarCep(cep) {
    ...
    // 1. Provedor 1: ViaCEP (Padrão ouro nacional)
    try {
      const res = await this.fetchComTimeout(`https://viacep.com.br/ws/${cepLimpo}/json/`, 3000);
      if (res.ok) {
      ...
      console.warn('[LocationService] ViaCEP timeout/falha, tentando BrasilAPI:', e.message);
    }

    // 2. Provedor 2: BrasilAPI
    try {
      const res = await this.fetchComTimeout(`https://brasilapi.com.br/api/cep/v2/${cepLimpo}`, 3000);
      if (res.ok) {
        const dados = await res.json();
        if (dados.state && dados.city) {
          return {
            uf: dados.state,
            estado: dados.state,
      ...
      console.warn('[LocationService] BrasilAPI falha, tentando OpenCEP:', e.message);
    }

    // 3. Provedor 3: OpenCEP
    try {
      const res = await this.fetchComTimeout(`https://opencep.com/v1/${cepLimpo}`, 3000);
      if (res.ok) {
        const dados = await res.json();
        if (dados.uf && dados.localidade) {
          return {
            uf: dados.uf,
            estado: dados.uf,
            localidade: dados.localidade,
      ...
      console.warn('[LocationService] OpenCEP falha, tentando AwesomeAPI:', e.message);
    }

    // 4. Provedor 4: AwesomeAPI
    try {
      const res = await this.fetchComTimeout(`https://cep.awesomeapi.com.br/json/${cepLimpo}`, 3000);
      if (res.ok) {
        const dados = await res.json();
        if (dados.state && dados.city) {
          return {
      ...
      console.warn('[LocationService] AwesomeAPI falha final:', e.message);
    }

    throw new Error('Não foi possível localizar o endereço para este CEP. Por favor, preencha manualmente.');
  }
  ...
  async getEstados() {
    ...
    try {
      const response = await this.fetchComTimeout('https://brasilapi.com.br/api/ibge/uf/v1', 2500);
      if (response.ok) {
      ...
      console.warn('[LocationService] BrasilAPI estados falhou, tentando IBGE:', err.message);
    }

    try {
      const response = await this.fetchComTimeout('https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome', 2500);
      if (response.ok) {
        const dados = await response.json();
      ...
      console.warn('[LocationService] Usando fallback local para estados:', err.message);
    }
  ...
  async getCidadesPorEstado(uf) {
    ...
    try {
      const response = await this.fetchComTimeout(`https://brasilapi.com.br/api/ibge/municipios/v1/${uf}`, 2500);
      if (response.ok) {
        const dados = await response.json();
        const cidades = dados
      ...
      console.warn('[LocationService] Falha BrasilAPI cidades, tentando IBGE:', err.message);
    }

    try {
      const response = await this.fetchComTimeout(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${uf}/municipios?orderBy=nome`, 2500);
      if (response.ok) {
        const dados = await response.json();
      ...
      console.warn(`[LocationService] Fallback local cidades para ${uf}:`, err.message);
    }
  ...
  async getBairros(estado, cidadeNome = '', cidadeId = null) {
    ...
    if (cidadeId && String(cidadeId).length > 3) {
      try {
        const response = await this.fetchComTimeout(`https://servicodados.ibge.gov.br/api/v1/localidades/municipios/${cidadeId}/distritos`, 2000);
        if (response.ok) {
          const dados = await response.json();
  ...
  async obterBairrosDistritos(cidadeId, cidadeNome = '') {
    return this.getBairros(null, cidadeNome, cidadeId);
  }

  /**
   * Obtém ruas / logradouros mapeados para a cidade e bairro selecionados
   */
  async getRuas(cidadeNome, bairroNome) {
  ...
  async obterRuasPorBairro(cidadeNome, bairroNome) {
    return this.getRuas(cidadeNome, bairroNome);
  }
}
